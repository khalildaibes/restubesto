...
export interface CartItem {
  ...
  imageUrl: string
  qty: number
  selectedIngredients?: CartItemIngredient[] // Selected optional ingredients
}

interface CartStore {
  items: CartItem[]
  addItem: (item: Omit<CartItem, 'qty'>) => void
  updateQty: (mealId: string, qty: number, selectedIngredients?: CartItemIngredient[]) => void
  removeItem: (mealId: string, selectedIngredients?: CartItemIngredient[]) => void
  clearCart: () => void
...
export const useCartStore = create<CartStore>((set, get) => ({
  items: [],
  addItem: (item) => {
    // If item has selectedIngredients, use addItemWithIngredients logic
    if (item.selectedIngredients && item.selectedIngredients.length > 0) {
      get().addItemWithIngredients(item)
      return
    }
    
    // Otherwise, match only by mealId (no customizations)
    ...
    if (existingItem) {
      set({
        items: get().items.map((i) =>
          i.mealId === item.mealId && (!i.selectedIngredients || i.selectedIngredients.length === 0)
            ? { ...i, qty: i.qty + 1 }
            : i
        ),
      })
  ...
  updateQty: (mealId, qty, selectedIngredients) => {
    if (qty <= 0) {
      ...
      if (item) {
        set({
          items: items.map((i) =>
            i.mealId === mealId && JSON.stringify(i.selectedIngredients || []) === itemKey
              ? { ...i, qty }
              : i
          ),
        })
  ...
  removeItem: (mealId, selectedIngredients) => {
    ...
    set({ 
      items: get().items.filter(
        (i) => !(i.mealId === mealId && JSON.stringify(i.selectedIngredients || []) === itemKey)
      )
    })
  },
  clearCart: () => {
